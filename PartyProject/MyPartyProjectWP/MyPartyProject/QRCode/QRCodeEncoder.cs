...
namespace Huyn.QRCode
{
    public class QRCodeGenerator
    {
        ...
        internal static String DATA_PATH = "/QRCode/datas";
        ...
        private bool[][] CalQrcode(byte[] qrcodeData)
        {
            ...
            try
            {
                Stream stream = GetResourceFile(DATA_PATH + "/" + "qrvfr" + Convert.ToString(qrcodeVersion) + ".dat");
        ...
        public WriteableBitmap Generate(String content)
        {
            return Generate(content, Encoding.UTF8);
        }

        internal static Stream GetResourceFile(string fileName)
        {
            Uri uri = new Uri("Huyn.QRCode;component/" + fileName, UriKind.Relative);
        ...
        private static int Read(System.IO.Stream sourceStream, sbyte[] target, int start, int count)
        {
            if (target.Length == 0)
                return 0;
